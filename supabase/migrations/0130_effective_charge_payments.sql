-- Record payments against the effective amount after closed-period adjustments.
-- This is the complete function definition; no dynamic function-body patching.
...
begin
  ...
  select coalesce(sum(item.amount), 0)::numeric(12,2) into v_allocated_amount
  ...
  ) then raise exception 'Every allocation must have a distinct charge and positive amount'; end if;

  v_change_amount := (p_received_amount - v_allocated_amount)::numeric(12,2);
  if v_change_amount < 0 then raise exception 'The received amount cannot be less than the allocated amount';
  elsif p_payment_method <> 'cash' and v_change_amount <> 0 then
    raise exception 'Only cash payments can include change';
  end if;

  select coalesce(jsonb_agg(jsonb_build_object(
    'charge_id', item.charge_id, 'amount', item.amount::numeric(12,2)
  ...
  from public.payments payment where payment.idempotency_key = p_idempotency_key;
  if v_payment_id is not null then
    ...
    elsif v_existing_shop_id <> p_shop_id or v_existing_fingerprint is distinct from v_request_fingerprint then
  ...
  select profile.* into v_profile from public.shop_payment_profiles profile where profile.shop_id = p_shop_id;
  if v_profile.id is null then raise exception 'The selected shop does not have a payment profile';
  ...
  elsif ((p_payment_method = 'cash' and v_profile.cash_reference_required)
  ...
  elsif ((p_payment_method = 'cash' and v_profile.cash_evidence_required)
    ...
    where evidence.bucket_id = 'payment-evidence' and evidence.name = trim(p_evidence_path)
      and (storage.foldername(evidence.name))[1] = auth.uid()::text
  ) then raise exception 'Payment evidence does not exist or does not belong to the current user';
  end if;
  ...
  if p_collection_run_id is not null then
    ...
    if v_collection_service_date is null then
      ...
      where charge.id is null or charge.shop_id is distinct from p_shop_id
        or charge.status is distinct from 'active'
        or not public.is_charge_collectible_in_run(charge.id, p_collection_run_id)
    ) then raise exception 'The payment contains a charge outside the caller''s assigned collection scope'; end if;
  elsif exists (
    select 1 from jsonb_to_recordset(p_allocations) requested(charge_id uuid, amount numeric)
    left join public.delivery_charges charge on charge.id = requested.charge_id
    where charge.id is null or charge.shop_id is distinct from p_shop_id
      or charge.status is distinct from 'active' or charge.payment_term is distinct from 'immediate'
      or not public.is_financial_charge_visible(charge.id)
  ) then raise exception 'The payment contains a charge outside the caller''s assigned scope';
  end if;
  ...
  where charge.shop_id = p_shop_id and charge.status = 'active'
    and (
      (p_collection_run_id is null and exists (
        select 1 from jsonb_to_recordset(p_allocations) requested(charge_id uuid, amount numeric)
        where requested.charge_id = charge.id
      ))
      or (p_collection_run_id is not null and public.is_charge_collectible_in_run(charge.id, p_collection_run_id))
    );
  ...
  loop
    if not exists (
      ...
      where charge.id = v_allocation.charge_id and charge.shop_id = p_shop_id and charge.status = 'active'
    ) then raise exception 'Every allocation must target an active charge for the selected shop';
    ...
    ) then raise exception 'An allocation cannot exceed the latest charge balance'; end if;
  end loop;
  ...
  if not v_profile.allow_outstanding and v_remaining_outstanding > 0 and exists (
    ...
    select charge.id, event.round_stop_id into v_outstanding_charge_id, v_outstanding_round_stop_id
    from jsonb_to_recordset(p_allocations) item(charge_id uuid, amount numeric)
    join public.delivery_charges charge on charge.id = item.charge_id
    join public.delivery_events event on event.id = charge.delivery_event_id
    where charge.payment_term = 'immediate';
    v_approval_fingerprint := md5(jsonb_build_object(
      'kind', 'outstanding_balance', 'charge_id', v_outstanding_charge_id,
      'outstanding_amount', v_remaining_outstanding
    )::text);
    select approval.* into v_approval from public.financial_approval_requests approval
    ...
    ) then raise exception 'Financial approval has expired'; end if;
  elsif p_approval_id is not null then raise exception 'This payment does not require a financial approval'; end if;
  ...
  values (auth.uid(), 'payments', v_payment_id, 'created', jsonb_build_object(
    'shop_id', p_shop_id, 'payment_method', p_payment_method,
    'received_amount', p_received_amount::numeric(12,2),
    'allocated_amount', v_allocated_amount, 'change_amount', v_change_amount,
    'allocations', p_allocations, 'collection_run_id', p_collection_run_id,
