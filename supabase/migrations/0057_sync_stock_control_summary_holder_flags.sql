-- Keep the stock-control summary's holder flags authoritative.  The current
-- wrapper adds presentation metadata, so it must also refresh these flags from
-- stock_locations instead of trusting a value carried by an older summary.
