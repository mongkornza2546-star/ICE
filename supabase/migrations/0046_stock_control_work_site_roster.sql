-- Expose the responsible employees for report-only work sites in the stock-control
-- summary. Work sites intentionally remain outside the inventory ledger.

alter function public.get_stock_control_summary(uuid, date)
  rename to get_stock_control_summary_v2;
...
revoke all on function public.get_stock_control_summary_v2(uuid, date) from public, authenticated;
