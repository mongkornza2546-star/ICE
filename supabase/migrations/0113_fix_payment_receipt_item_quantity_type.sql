-- delivery_items.quantity became numeric(12,1) in 0107 to support half-bags.
-- The receipt RPC was added later but declared its fifth result column as an
-- integer, causing PostgreSQL to reject every receipt-detail query at runtime.
-- A return type change requires dropping and recreating the function.
drop function public.get_payment_receipt_items(uuid);
