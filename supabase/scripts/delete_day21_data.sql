...
-- สำหรับวันที่ 21 กรกฎาคม 2569 (21/07/2026)
--
-- วิธีใช้:
...
WHERE s.service_date = '2026-07-21'
...
WHERE r.service_date = '2026-07-21'
...
BEGIN;
  ...
  -- 2A. ลบ stock_movement_items (child ของทุก movement)
  ...
  -- 2M. ลบ round_stock_snapshot_items → round_stock_snapshots (cascade ได้ แต่ลบเผื่อความชัดเจน)
  ...
  WHERE entity_type IN ('delivery_rounds', 'delivery_events', 'round_stops')
    AND entity_id IN (
      SELECT r.id FROM public.delivery_rounds r WHERE r.service_date = '2026-07-21'
      UNION
...
ROLLBACK;
-- ⚠️ เปลี่ยนเป็น COMMIT เมื่อตรวจสอบแล้วว่าถูกต้อง
