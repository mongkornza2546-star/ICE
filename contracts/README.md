...
result for every command type plus the cross-runtime fingerprint values. A
change to an existing v1 field, result, error code, canonical value, or digest
is a breaking change. Add a new contract version instead of rewriting a queued
command's meaning.
