...
import { renderSalesDocumentRaster } from './thermalReceiptRaster';
import {
...
export async function printSalesDocumentForCurrentPlatform(
  payload: SalesDocumentPayload,
  existingPrintWindow?: Window | null,
) {
  if (!isAndroidApp()) return printSalesDocument(payload, existingPrintWindow);
  existingPrintWindow?.close();
  const imageBase64 = await renderSalesDocumentRaster(payload);
